...
public class StringUtils {
     ...
     * @return
     */
    public static String convertDot(String name) {
        if(isEmpty(name)) {
            return "";
        } else {
    ...
    /**
     * 隐藏身份证号，只留第一位和最后一位，其余用*代替
     * @param credentialNo
     * @return
     */
    public static String hideCredentialNo(String credentialNo) {
        return credentialNo.substring(0, 1) + "****************" + credentialNo.substring(credentialNo.length() - 1);
    }
     ...
     * @return
     */
    public static String getLast4Digits(String cardStr){
        if(isEmpty(cardStr)){
            return "";
        }
        if(cardStr.length() < 4){//小于四位，直接返回
            return cardStr;
        }
